...
toGenericAttribute (key, a) = G.Attribute
    ...
    , G.attributeDisplayName      = DB.attributeDisplayName a
    , G.attributeGroup            = DB.attributeGroup a
...
toGQLItem
    :: (Key DB.Item, DB.Item, Maybe Double)  -- ^ The database item and distance
    -> GQL.Item      -- ^ The GQL item
toGQLItem (key, item, d) = GQL.Item
    { GQL.itemId          = Just $ keyToID key
...
toGenericItem
    :: (Key DB.Item, DB.Item, Maybe Double)   -- ^ The database item
    -> G.Item                            -- ^ The Generic item
toGenericItem (key, item, d) = G.Item
    { G.itemId          = Just $ keyToText key
...
toDataItem item = DB.Item
    ...
    , DB.itemPosition    = Position $ PointXY
                               (realToFrac $ GQL.itemLatitude item)
                               (realToFrac $ GQL.itemLongitude item)
    }

-- | Converts a Database user to a generic user
toGenericUser :: (Key DB.User, DB.User) -> G.User
toGenericUser (k, u) = G.User { G.userId       = Just $ keyToText k
