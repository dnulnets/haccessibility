...
--
module Accessability.Handler.GQL
   ( postGQLR
   )
where
...
import           Accessability.Model.Transform  ( idToKey
                                                , toGQLItem
                                                )
...
resolveUpdateItem arg =
   fffmap toGQLItem liftEither
      ...
      <> DBF.changeField
            DB.ItemPosition
            (maybePosition (realToFrac <$> updateItemLongitude arg)
                           (realToFrac <$> updateItemLatitude arg)
            )
...
resolveCreateItem arg = ffmap toGQLItem liftEither $ DBF.dbCreateItem $ DB.Item
   ...
   , DB.itemPosition    = Position $ PointXY
                             (realToFrac $ createItemLongitude arg)
                             (realToFrac $ createItemLatitude arg)
   }
...
resolveItems args = fffmap toGQLItem liftEither $ DBF.dbFetchItems
   ...
   (maybePosition (realToFrac <$> queryItemsLongitude args)
                  (realToFrac <$> queryItemsLatitude args)
   )
...
gqlApi
   :: GQLRequest         -- ^ The graphql request
