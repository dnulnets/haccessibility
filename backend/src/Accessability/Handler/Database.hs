...
dbFetchItemAttributes key = do
 ...
 where
  cleanup
     :: (Entity Attribute, Entity AttributeValue)
     -> (Key Attribute, Attribute, Key AttributeValue, AttributeValue)
...
dbFetchItems
   :: Maybe Text                         -- ^ Part of name or description
...
dbFetchItems t p d limit = do
   item <- case p of
      Just _ -> do
         i1 <- runDB $ rawSql (sqlFetchBuildQuery t p d limit)
                              (sqlFetchPlaceholder t p d limit)
         return $ clean1 <$> i1
      Nothing -> do
         i2 <- runDB $ rawSql (sqlFetchBuildQuery t p d limit)
                              (sqlFetchPlaceholder t p d limit)
         return $ clean2 <$> i2
   return $ Right item
 where
  ...
  sqlFetchBuildQuery
     :: Maybe Text
     -> Maybe GeospatialPosition
     -> Maybe Double
     -> Maybe Int
     -> Text
  sqlFetchBuildQuery Nothing Nothing _ l =
     "SELECT ?? FROM item ORDER BY name" <> sqlFetchLimit l
  sqlFetchBuildQuery (Just _) Nothing _ l =
     "SELECT ?? FROM item WHERE (name ILIKE ? OR description ILIKE ?) ORDER BY name"
        <> sqlFetchLimit l
  sqlFetchBuildQuery (Just _) (Just _) Nothing l =
     "SELECT ??, ST_Distance(position, ?, true) FROM item WHERE (name ILIKE ? OR description ILIKE ?) ORDER BY ST_Distance(position, ?, true)"
        <> sqlFetchLimit l
  sqlFetchBuildQuery (Just _) (Just _) (Just _) l =
     "SELECT ??, ST_Distance(position, ?, true) FROM item WHERE (name ILIKE ? OR description ILIKE ?) AND ST_DWithin(position,?,?,true) ORDER BY ST_Distance(position, ?, true)"
        <> sqlFetchLimit l
  sqlFetchBuildQuery Nothing (Just _) (Just _) l =
     "SELECT ??, ST_Distance(position, ?, true) FROM item WHERE ST_DWithin(position,?,?,true) ORDER BY ST_Distance(position, ?, true)"
        <> sqlFetchLimit l
  sqlFetchBuildQuery Nothing (Just _) Nothing l =
     "SELECT ??, ST_Distance(position, ?, true) FROM item ORDER BY ST_Distance(position, ?, true)"
        <> sqlFetchLimit l
  ...
  sqlFetchPlaceholder (Just tt) Nothing _ l =
     [PersistText tt, PersistText tt] <> sqlFetchLimitPlaceholder l
  sqlFetchPlaceholder (Just tt) (Just pp) Nothing l =
     [toPersistValue pp, PersistText tt, PersistText tt, toPersistValue pp]
        <> sqlFetchLimitPlaceholder l
  sqlFetchPlaceholder (Just tt) (Just pp) (Just dd) l =
     [ toPersistValue pp
        , PersistText tt
        , PersistText tt
        , toPersistValue pp
        ...
        <> sqlFetchLimitPlaceholder l
  sqlFetchPlaceholder Nothing (Just pp) (Just dd) l =
     [toPersistValue pp, toPersistValue pp, PersistDouble dd, toPersistValue pp]
        <> sqlFetchLimitPlaceholder l
  sqlFetchPlaceholder Nothing (Just pp) Nothing l =
     [toPersistValue pp, toPersistValue pp] <> sqlFetchLimitPlaceholder l
...
dbUpdateItemAttributes
   :: [(Maybe (Key AttributeValue), Maybe AttributeValue)]
...
dbUpdateItemAttributes aav = do
 ...
 where
  ...
  generate (Just k, Just av) =
     update k [AttributeValueValue =. attributeValueValue av]
