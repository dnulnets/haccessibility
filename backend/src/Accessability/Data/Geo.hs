...
longitude
  :: GeospatialPosition -- ^ The geospatial position
...
geoPoint
  :: GeospatialGeometry -- ^ The geospatial gemoetry
...
instance PersistField GeospatialPosition where
  toPersistValue (Position t) =
    PersistDbSpecific
      $ encodeUtf8
      $ toText
      $ fromBytes
      $ toStrict
      $ toByteString LittleEndian defaultSRID (Point (GeoPoint (GeoPointXY t)))
...
--    either (Left . pack) Right $ (Position . geoPoint) <$> parseHexByteString (Hex t)
  fromPersistValue _ =
    Left "Position values must be converted from PersistDbSpecific"
...
instance PersistField Geospatial where
  toPersistValue (Geometry t) =
    PersistDbSpecific $ toStrict $ toByteString LittleEndian defaultSRID t

  fromPersistValue (PersistDbSpecific t) =
    either (Left . Data.Text.pack) Right $ Geometry <$> parseHexByteString
      (Hex t)
  fromPersistValue _ =
